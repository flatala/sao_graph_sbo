...
class MDGNCCalculator:
    """
    ADORE-side evaluator logic for the mixed-discrete GNC benchmark from SBArchOpt.

    Each instantiated component gets a continuous sizing parameter in [0, 1] that
    determines both its mass and failure rate. Connectivity remains discrete and
    hierarchical.
    """
    ...
    @staticmethod
    def _calc_failure_rate_from_arrays(failure_rates, obj_conns):
        conn_matrices = []
    ...
    def plot_results(results_df: pd.DataFrame, evaluator_name: str):
        ...
        plt.figure(), plt.title(f"MD GNC Problem Results: {evaluator_name}")
...
class MDGNCEvaluator(GraphApiEvaluator):
    """
    ...
    def _evaluate(self, architecture: Architecture, arch_qois: List[ArchQOI], **_) -> Dict[ArchQOI, float]:
        ...
        for arch_qoi in arch_qois:
            if arch_qoi.ref == "mass":
                arch_qoi_map[arch_qoi] = MDGNCCalculator.calc_mass(
                    sensor_params,
                    computer_params,
                ...
                arch_qoi_map[arch_qoi] = MDGNCCalculator.calc_failure_rate(
                    sensor_params,
                    computer_params,
                    sensor_computer_conns,
                    actuator_params,
        ...
        for component in architecture.system.components:
            ...
            for instance in component.instances:
                if len(instance.qois) == 0:
                    continue
                param = float(instance.qois[0].value)
                params.append(param)
        return params
...
def get_evaluator(path):
    return MDGNCEvaluator.from_file(str(path), save_to_project=True)
