...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Submit an experiment grid as one Slurm task per seed run.")
    parser.add_argument(
        "experiment_file",
        type=Path,
...
def _load_file(path: Path):
    ...
    module_name = f"_experiment_submit_{experiment_path.stem}"
...
def main() -> int:
    ...
    n_runs = int(_required(module, "N_RUNS"))
    ...
    if runs_per_task <= 0:
        raise SystemExit(f"SLURM_RUNS_PER_TASK must be positive, got {runs_per_task}")
    ...
    for config_index, config in enumerate(configs):
        ...
        for run_id in range(n_runs):
            run_config_path = experiment_dir / f"run_{run_id:03d}" / "run_config.json"
            if run_config_path.exists():
                with run_config_path.open("r", encoding="utf-8") as fh:
                    run_config = json.load(fh)
                if run_config.get("status") == "completed":
                    expected_seed = int(module.BASE_SEED) + run_id
                    if run_config.get("seed") != expected_seed:
                        raise SystemExit(
                            f"Completed run has unexpected seed in {run_config_path}: "
                            f"{run_config.get('seed')!r} != {expected_seed!r}"
                        )
                    continue
            pending_tasks.append({"config_index": config_index, "run_id": run_id})

    if len(pending_tasks) == 0:
        print("No pending runs to submit.")
        return 0

    if runs_per_task == 1:
        array_tasks = pending_tasks
    else:
        array_tasks = [{"tasks": task_group} for task_group in _chunks(pending_tasks, runs_per_task)]

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    if array_chunk_size is None:
        task_chunks = [array_tasks]
        manifest_paths = [log_dir / f"pending_tasks_{timestamp}.json"]
    else:
        task_chunks = [
            array_tasks[i:i + array_chunk_size]
            for i in range(0, len(array_tasks), array_chunk_size)
        ]
        manifest_paths = [
            log_dir / f"pending_tasks_{timestamp}_chunk_{chunk_index:03d}.json"
    ...
    for task_chunk, manifest_path in zip(task_chunks, manifest_paths):
        ...
        command = ["sbatch", "-D", str(workdir), "-J", job_name, "-o", str(log_dir / "%x_%A_%a.out"), "-e",
                   str(log_dir / "%x_%A_%a.err"), "--export=NONE", f"--array={array_spec}",
                   f"--cpus-per-task={cpus_per_task}", f"--mem={mem}", f"--mail-user={mail_user}",
                   f"--mail-type={mail_type}", f"--time={slurm_time}"]

        if clusters:
            command.append(f"--clusters={clusters}")
        if partition:
        ...
        command.append(str(args.experiment_file))
        command.append(str(manifest_path))
        command.append(str(micromamba_env))
        command.append(str(micromamba_module_use))
        command.append(str(micromamba_module))
        commands.append((array_spec, manifest_path, command))

    print("Submitting experiment grid:")
    print(f"  configs: {len(configs)}")
    print(f"  runs_per_config: {n_runs}")
    print(f"  pending_tasks: {len(pending_tasks)}")
    print(f"  runs_per_slurm_task: {runs_per_task}")
    print(f"  task_parallelism: {task_parallelism}")
    print(f"  slurm_tasks: {len(array_tasks)}")
    ...
    print(f"  micromamba_env: {micromamba_env}")
    print(f"  micromamba_module_use: {micromamba_module_use}")
    print(f"  micromamba_module: {micromamba_module}")
    print("  commands:")
