...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Submit a GNN encoder grid as GPU-local config chunks.")
...
def _load_file(path: Path):
    ...
    module_name = f"_gnn_encoder_submit_{experiment_path.stem}"
...
def main() -> int:
    ...
    configs_per_task = int(_required(module, "CONFIGS_PER_TASK"))
    parallel_per_gpu = int(_required(module, "PARALLEL_CONFIGS_PER_GPU"))
    ...
    array_concurrency = getattr(module, "SLURM_ARRAY_CONCURRENCY", None)

    if configs_per_task <= 0:
        raise SystemExit(f"CONFIGS_PER_TASK must be positive, got {configs_per_task}")
    if parallel_per_gpu <= 0:
        raise SystemExit(f"PARALLEL_CONFIGS_PER_GPU must be positive, got {parallel_per_gpu}")
    if cpus_per_task <= 0:
    ...
    from experimenter.encoder_training.grid import config_result_dir
    ...
    for config_index, config in enumerate(configs):
        ...
        if run_config_path.exists():
            ...
            if run_config["status"] == "completed":
                continue
        pending_indices.append(config_index)

    if len(pending_indices) == 0:
        print("No pending GNN encoder fits to submit.")
        return 0

    pending_tasks = [{"config_indices": chunk} for chunk in _chunks(pending_indices, configs_per_task)]
    manifest_path = log_dir / f"pending_gnn_encoder_tasks_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    with manifest_path.open("w", encoding="utf-8") as fh:
        json.dump(pending_tasks, fh, indent=2)

    sbatch_script = Path(__file__).with_name("run.sbatch")
    array_spec = f"0-{len(pending_tasks) - 1}"
    ...
    print("Submitting GNN encoder grid:")
    print(f"  configs: {len(configs)}")
    print(f"  pending_configs: {len(pending_indices)}")
    print(f"  configs_per_task: {configs_per_task}")
    print(f"  parallel_per_gpu: {parallel_per_gpu}")
    print(f"  gpu_tasks: {len(pending_tasks)}")
    print(f"  array: {array_spec}")
    print(f"  cpus_per_task: {cpus_per_task}")
    print(f"  mem: {mem}")
    print(f"  gres: {gres}")
    print(f"  estimated_waves_per_gpu_task: {math.ceil(configs_per_task / parallel_per_gpu)}")
